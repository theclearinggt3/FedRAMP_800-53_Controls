...
names(Controls_mod) <- c("Control_Family","Family_Name","Control_Name", "Main_Control","Control_Enhancement_Name", "Control_Enhancement","Combined","Related_Controls","Low","Low_2","Medium","Medium_2","High","High_2","Priority","Impact")
...
#This provideds a count "Weight" on the duplicates
...
#write_csv(Gephi_Controls4, "Gephi_Controls4.csv") #exporting the Node file into a csv file called Nodes.csv into your working directory you set earlier.
...
```{r}
#Create Node Table
#Nodes_temp_MC <- distinct(Gephi_Controls6, Main_Control)
#names(Nodes_temp_MC)<- c("ID")

#Nodes_temp_RC <- distinct(Gephi_Controls6, Related_Controls)
...
Nodes <- distinct(Controls, Control_Family, Family_Name, Main_Control, Control_Name)
  
Nodes["Name"] <- Nodes$Main_Control 

Nodes <- Nodes %>%
  select (Main_Control, Name, Control_Name, Control_Family, Family_Name) #%>%
  #rename(Main_Control = Id) 

Nodes <- rename(Nodes, c("Main_Control" = "Id"))
...
#Create Edges Table
Edges <- select(Gephi_Controls6, Main_Control, Related_Controls, Weight)
